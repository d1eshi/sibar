...
function inferArtifactType(artifactKind: string): WorkspaceArtifactKind {
  if (artifactKind === "code_slice" || artifactKind === "patch_preview") {
    return "code";
  }
  if (artifactKind.includes("paper") || artifactKind.includes("hypothesis")) {
    return "paper";
  }
  if (artifactKind.includes("memory") || artifactKind.includes("review")) {
    return "text";
  }
  if (artifactKind.includes("flow") || artifactKind.includes("architecture") || artifactKind.includes("concept")) {
    return "markdown";
  }
  return "unknown";
}

function readArtifactPayloadPath(payload: unknown): string | null {
...
function readArtifactRange(payload: unknown): { start: number; end: number } | null {
  if (!payload || typeof payload !== "object") {
    return null;
  }
...
function buildEvidenceContracts(
  ...
  return evidenceInventory.map((evidence): EvidenceContract => {
    ...
    return {
      ...
      title,
      line_range: {
        line_start: evidence.line_count ? 1 : 1,
        line_end: evidence.line_count ?? 1,
      },
...
function buildArtifactPreviews(loop: DeepOwnershipLoop): ArtifactPreviewContract[] {
  return loop.thinking_artifacts.map((artifact) => {
    ...
    const payloadPath = readArtifactPayloadPath(artifact.payload);
    const path = payloadPath ?? fallbackPath;
    const range = artifact.payload ? readArtifactRange(artifact.payload) : null;
    const excerpt = artifact.payload && typeof artifact.payload === "object"
    ...
    return {
      ...
      artifact_type: inferArtifactType(artifact.kind),
      language: typeof language === "string" ? language : null,
      excerpt: typeof excerpt === "string" ? excerpt : null,
      slice_content: null,
      line_start: range?.start ?? null,
      line_end: range?.end ?? null,
      preview_fallback_reason: artifact.success_criteria.length === 0 ? "no success criteria provided" : null,
...
export function buildWorkspaceSessionContract(input: {
  ...
  const evidence = buildEvidenceContracts(input.session.loop.evidence_inventory, requiredEvidenceIds);
