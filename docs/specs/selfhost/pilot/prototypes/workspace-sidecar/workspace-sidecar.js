const fixture = window.workspaceFixture;
...
const pathInput = byID("pathInput");
...
function renderRepoList() {
  ...
  `);
    ...
    repoList.querySelectorAll(".repo-item").forEach((button) => {
...
function selectNode(nodeID) {
    ...
    nodeList.querySelectorAll(".node-card").forEach((entry) => {
        entry.classList.toggle("active", entry.dataset.nodeId === nodeID);
    });
...
function evaluateAnswer() {
    ...
    const mentionsEvidence = fixture.manifest.included_paths.some((path) => lower.includes(path.toLowerCase()) || lower.includes(shortPath(path).split("/").at(-1).toLowerCase()));
    ...
    if (declaresUncertainty || !mentionsBoundary) {
        ...
        renderAnswerResult("gap", "Gap: needs evidence-producing repair", gap
    ...
    if (!mentionsEvidence) {
        renderAnswerResult("gap", "Evidence gap", "The explanation may be plausible, but this prototype needs at least one in-bound file citation before Sibi can claim readiness.");
        renderPractice();
        return;
    }
    renderAnswerResult("ready", "Readiness: bounded and evidence-backed", "This answer names the command/evidence boundary and cites in-scope files. Sibi can treat it as ready to explain this slice, not whole-repo ownership.");
}
function renderAnswerResult(kind, title, body) {
