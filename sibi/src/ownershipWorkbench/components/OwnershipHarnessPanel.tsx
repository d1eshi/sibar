import type { AttemptResult, BoundaryState, EvidenceRef, OwnershipBoundary } from "../types";
      ...
      <section className="ownershipSection">
        ...
        <ul>
          {evidenceRefs.map((entry) => (
            <li key={entry.id}>
              <span className={`evidenceBadge ${entry.confidence}`}>{entry.confidence}</span> {entry.title}
